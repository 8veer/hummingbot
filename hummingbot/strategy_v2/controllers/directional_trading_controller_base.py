...
class DirectionalTradingControllerConfigBase(ControllerConfigBase):
    ...
    connector_name: str = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the name of the exchange to trade on (e.g., ibkr):"))
    trading_pair: str = Field(
        default="RELIANCE-INR",
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the trading pair to trade on (e.g., RELIANCE-INR):"))

    total_amount_quote: Decimal = Field(
        default=1000.0,
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the amount of quote asset to use per executor (e.g., 1000):"))
